...
#define sensorlinhaanalogico1 36
#define sensorlinha2 8
#define sensorlinhaanalogico2 35
#define sensorlinha3 4
#define sensorlinhaanalogico3 34
#define sensorlinha4 13
#define sensorlinhaanalogico4 36
...
uint16_t verdeNoSensor1;
uint16_t verdeNoSensor2;
...
void seguelinha() {
  ...
  // Nenhum sensor detecta a linha, segue a vida do robô que pode ser gap
    ...
    (digitalRead(sensorlinha1) == 0 && digitalRead(sensorlinha2) == 0 && digitalRead(sensorlinha3) == 0 && digitalRead(sensorlinha4) == 0) {
      frente();
    }
...
void verificaVerde() {
  ...
  // Verifica a presença de marcações verdes nos sensores
   verdeNoSensor1 = corVerde1();
   verdeNoSensor2 = corVerde2();
  ...
  if (verdeNoSensor1 || verdeNoSensor2) {
    ...
    bool linhaPresente = digitalRead(sensorlinha1) == 0 || digitalRead(sensorlinha2) == 0 || digitalRead(sensorlinha3) == 0 || digitalRead(sensorlinha4) == 0;
...
void verificaEntradaResgate() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (leituraSensor > 50) {  // Substitua LIMITE_REFLEXAO pelo seu valor de limite
      linhaDetectada = true;
  ...
  if (linhaDetectada && (distanciaLateral1 < 90 && distanciaLateral2 < 90)) {
